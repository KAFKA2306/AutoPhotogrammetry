...
このPythonスクリプトは、様々なフォトグラメトリーソフトウェアを使用して画像セットから3Dモデルを生成し、生成されたモデルの品質を構造的類似度指数（SSIM）を用いて評価する処理を自動化します。

#### 特徴

- 必要なフォトグラメトリーソフトウェアがインストールされていない場合、自動的にインストール
- 複数のフォトグラメトリーソフトウェア（Meshroom、VisualSFM、COLMAP）をサポート
- 画像ディレクトリから3Dモデルを生成
- SSIMを使用して生成されたモデルの品質を評価
- クロスプラットフォーム互換性（WindowsとLinux）


#### サポートされているフォトグラメトリーソフトウェア
...
- `main.py`: 画像収集、クラスタリング、生成、選別の一連の処理を実行します。
- `image_collector.py`: 画像の収集を行います。
- `image_clustering.py`: 画像のクラスタリングを行います。
- `image_generator.py`: 画像の生成を行います。
- `image_selector.py`: 画像の選別を行います。
- `config.py`: APIキーや出力ディレクトリなどの設定を行います。
- `requirements.txt`: 必要なPythonライブラリを記載しています。
