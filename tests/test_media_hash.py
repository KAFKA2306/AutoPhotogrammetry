...
def _registry(path: Path, *, sha256: str | None = None, expected_size: int | None = 3) -> None:
    source = {
        ...
        "author": "author",
        "license": {"name": "CC0", "status": "verified", "url": "https://creativecommons.org/publicdomain/zero/1.0/"},
        "duration_seconds": 1,
        "resolution": [1, 1],
...
def test_sha256_stream_hashes_exact_bytes() -> None:
    digest, size = sha256_stream(io.BytesIO(b"abc"), chunk_size=2)
    assert digest == hashlib.sha256(b"abc").hexdigest()
    assert size == 3


def test_update_registry_source_hash_records_downloaded_bytes(tmp_path: Path) -> None:
    registry = tmp_path / "videos.json"
    _registry(registry)
    ...
    assert result == {"id": "sample", "sha256": expected, "size_bytes": 3}
    assert saved["sha256"] == expected
    assert saved["metadata_evidence"]["downloaded_size_bytes"] == 3
    assert saved["metadata_evidence"]["sha256_verified_from_downloaded_bytes"] is True


def test_update_registry_source_hash_refuses_existing_identity_drift(tmp_path: Path) -> None:
    registry = tmp_path / "videos.json"
    ...
    with patch("processing.media_hash.hash_source_media", return_value=(actual, 3)):
        with pytest.raises(ValueError, match="does not match downloaded bytes"):
