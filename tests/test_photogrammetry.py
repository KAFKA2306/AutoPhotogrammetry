...
from processing.photogrammetry import (
    BackendConfig,
...
class PhotogrammetryRunnerTests(unittest.TestCase):
    ...
    def test_missing_executable_fails_with_configuration_guidance(self):
        with patch(
            "processing.photogrammetry.shutil.which",
            return_value=None,
        ):
    ...
    def test_config_loading_stays_with_photogrammetry(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            config = load_backend_configs(path)["colmap"]
            self.assertEqual(config.executable, "/usr/bin/colmap")
            self.assertEqual(
                config.extra_args,
                ("--quality", "medium"),
            )
